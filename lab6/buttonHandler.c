...
void buttonHandler_runTest(int16_t touchCountArg) {
  ...
  // Draw all the buttons for the first time so the buttonHandler doesn't need
  // to do this in an init state. Ultimately, simonControl will do this when the
  // game first starts up.
  simonDisplay_drawAllButtons();
  buttonHandler_init(); // Initialize the button handler state machine
  buttonHandler_enable();
  while (touchCount < touchCountArg) { // Loop here while touchCount is less
                                       // than the touchCountArg
    buttonHandler_tick();              // Advance the state machine.
    utils_msDelay(RUN_TEST_TICK_PERIOD_IN_MS);
    if (buttonHandler_releaseDetected()) { // If a release is detected, then the
                                           // screen was touched.
  ...
  display_setCursor(
      TEXT_MESSAGE_ORIGIN_X,
      TEXT_MESSAGE_ORIGIN_Y); // Move the cursor to a rough center point.
  display_println(RUN_TEST_TERMINATION_MESSAGE1); // Print the termination
