...
int main() {
  ...
  uint32_t personalInterruptCount =
      0; // Used to compare with internal count from interrupt code.
  /******************** Game-Specific Code ********************/
  ...
  while (FOREVER) {              // Endless loop.
    wamMain_selectMoleCountFromSwitches(
        switches_read());          // Mole count selected via slide switches.
    ...
    while (!wamControl_isGameOver() &&
           !buttons_read()) {         // Game runs until over or interrupted.
      if (interrupts_isrFlagGlobal) { // If an interrupt occurs, time to call
                                      // tick.
