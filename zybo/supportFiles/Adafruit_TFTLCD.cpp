...
void Adafruit_TFTLCD::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  if (driver == ID_932X) {
    ...
    // This bug-fix was tested against all of the provided test routines and
    // looks to generate the correct output on the LCD. Start of bug-fix.
    ...
    case 1:                          // New code. BLH
    case 3:                          // New code. BLH
      // Landscape mode (1: origin in upper-left corner of 330 board, 3: origin
      // lower right corner.
...
uint16_t Adafruit_TFTLCD::readPixel(int16_t x, int16_t y) {
  ...
  if (driver == ID_932X) {
    ...
    return (((uint16_t)r & /*B11111000*/ 0xF8) << 8) |
           (((uint16_t)g & /*B11111100*/ 0xFC) << 3) | (b >> 3);
  } else
    return 0;
}
...
void Adafruit_TFTLCD::writeRegister8(uint8_t a, uint8_t d) {
  ...
  // This functionality was copied from pin_magic.h which is not included or
  // otherwise used here.
  LCD_setCommandMode();
  write8(a);
  LCD_setDataMode();
...
void Adafruit_TFTLCD::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
  ...
  // from pin_magic.h
  uint8_t hi = (d) >> 8, lo = (d); /*CD_COMMAND;*/
  LCD_setCommandMode();
  write8(aH); /*CD_DATA;*/
  LCD_setDataMode();
  write8(hi);
  /*CD_COMMAND;*/ LCD_setCommandMode();
  write8(aL); /*CD_DATA;*/
  LCD_setDataMode();
  write8(lo);
}
