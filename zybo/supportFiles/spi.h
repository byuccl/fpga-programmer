...
// Provides an API to use the AXI SPI controller provided by Xilinx. I am
// accessing the SPI controller directly and am not using any Xilinx driver
// code. I wanted to have better control over the SPI hardware. All of the
// register offsets, etc., are derived from the Xilinx SPI controller
// documentation: LogiCORE IP AXI Serial Peripheral Interface (AXI SPI)
// (v1.02.a), AKA, axi_spi_ds742.pdf
...
// All of the types below come directly from the SPI documentation provided by
// Xilinx.
...
#define SPI_RESET_REG_MASK                                                     \
  0x0000000A // Write this value to force a software reset of the SPI core.
...
#define SPI_DELAY_FUDGE_FACTOR                                                 \
  10000 // This is the multiplier to get the delay value of 1 to be 1
        // millisecond.

#define SPI_TFT_SLAVE_SELECT_MASK                                              \
  0x00000001 // TFT SPI slave select is bit 0 (only used if LCD is accessed via
             // SPI - deprecated).
#define SPI_TOUCH_SCREEN_CONTROLLER_SLAVE_SELECT_MASK                          \
  0x00000002 // Touch-screen controller slave select is bit 1.
#define SPI_BLUETOOTH_RADIO_SLAVE_SELECT_MASK                                  \
  0x00000004 // Bluetooth-radio slave-select is bit 3.

// The following named routines provide functionality necessary for the adafruit
// touch-controller code. Some of the functions are empty but are provided to
// minimize modifications to the adafruit code. See spi.c for functionality and
// comments.
void spi_begin(
    void); // init routine, used begin to mirror the adafruit source code.
