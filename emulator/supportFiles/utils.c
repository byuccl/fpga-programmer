...
void utils_msDelay(long msDelay) {
  ...
  // Convert the ms delay into seconds and nano seconds and load into the requestedDelay struct.
...
//  printf("util_msDelay() requesting %ld seconds and %ld ms.\n", seconds, msSeconds);
  ...
  while (nanosleep(&requestedDelay, &remainingDelay) == -1) {
    ...
    printf("utils_msDelay() nanosleep() retrying with %ld seconds and %ld nanoseconds.\n", 
...
// These busy-loops are in a separate thread (UserThread so going to sleep is ok.
void utils_wait() {
  utils_msDelay(WAIT_TIME_IN_MS);
}
