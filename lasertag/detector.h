...
#define DETECTOR_STATUS_FAILED_IGNORE_USER_FREQUENCY 2  // Didn't properly ignore a specific frequency.
#define DETECTOR_STATUS_FAILED_FIND_MAX_POWER 3         // Didn't find the max power frequency number.
#define DETECTOR_STATUS_FAILED_ADC_SCALING 4            // Didn't properly scale the ADC value.
...
// bool array is indexed by frequency number, array location set for true to ignore, false otherwise.
...
// if interruptsNotEnabled = true, interrupts are not running. If interruptsNotEnabled = true
...
// Ignore all hits. Used to provide some limited invincibility in some game modes.
// The detector will ignore all hits if the flag is true, otherwise will respond to hits normally.
...
// maxPowerFreqNo is the frequency number with the highest value contained in the unsortedValues.
// unsortedValues contains the unsorted values.
// sortedValues contains the sorted values.
// Note: it is assumed that the size of both of the array arguments is 10.
...
//detector_status_t detector_testSort(sortTestFunctionPtr testSortFunction, bool printTestMessages);
